...
def command_prompt():
    parser = argparse.ArgumentParser(description="CS50Filters")
    parser.add_argument("input", help="Input image file to apply filter on.")
    parser.add_argument(
        "filter", help="Filter to apply on the image.\nUse none to only change img format.", choices=available_filters.keys())
